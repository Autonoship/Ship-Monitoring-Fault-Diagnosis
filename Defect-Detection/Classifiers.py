...
def RF_train(feat, n_trees, max_feat):
    ...
    with open('Models/rf.pkl', 'wb') as f:
        pickle.dump(clf, f)
    print("Saved model to: Models/rf.pkl")
...
#     clf.fit(X_train, y_train)
#     with open('Models/xtra.pkl', 'wb') as f:
#         pickle.dump(clf, f)
#     print("Saved model to: Models/xtra.pkl")
#     # # and later you can load it
#     # with open('Models/xtra.pkl', 'rb') as f:
#     #     clf = pickle.load(f)
...
def GB(feat, n_est, max_feat, lrate):
    ...
    with open('Models/gb.pkl', 'wb') as f:
        pickle.dump(gb, f)
    print("Saved model to: Models/gb.pkl")
    # # and later you can load it
...
def CNN(dataset_loc, epoch, val_split):
    ...
    # # for loading Model/CNN.h5 model
...
def pretrained_CNN(data_dir):
    ...
    check_path = ".../cp2.ckpt" #Need to specify the .ckpt file location
